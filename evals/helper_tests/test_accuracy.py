#!/usr/bin/env python3
"""
Test script for accuracy metric.

NOTE: This is an interim test script used to verify the accuracy evaluation function
...
from evals.metrics import evaluate_accuracy
...
def main():
    ...
    print("\nEvaluating accuracy with LLM consensus...")
    ...
    print(f"YES votes (hallucinations detected): {result['yes_votes']}")
    print(f"NO votes (no hallucinations): {result['no_votes']}")
    print("\nVotes by model:")
    for model, vote_data in result['votes'].items():
        print(f"  {model}: {vote_data['vote']}")
        print(f"    Hallucinations: {vote_data['hallucinations']}")
    print("=" * 60)
    ...
    output_file = output_dir / "test_accuracy_results.json"
